...
public class RowToPubsubMessageTest implements Serializable {
  ...
  public void testConvertsMessages() {
    ...
    PCollection<PubsubMessage> rows =
        pipeline
            .apply(
                "create",
                Create.timestamped(
                  row(1, messageSchema, /* map("attr", "val"), */ row(payloadSchema, 3, "foo")),
                  row(2, messageSchema, /* map("bttr", "vbl"), */ row(payloadSchema, 5, "baz")),
                  row(3, messageSchema, /* map("cttr", "vcl"), */ row(payloadSchema, 7, "bar")),
                  row(4, messageSchema, /* map("dttr", "vdl"), */ row(payloadSchema, 8, "qaz")),
                  row(4, messageSchema, /* map("dttr", "vdl"), */ row(payloadSchema, null, null))))
            .apply(
                "convert",
                    RowToPubsubMessage.fromTableConfig(PubsubJsonTableProvider.PubsubIOTableConfiguration.builder().setSchema(messageSchema).setUseFlatSchema(true).build()));
  ...
  private TimestampedValue<Row> row(int timestamp, Schema schema, Object... objects) {
    return TimestampedValue.of(Row.withSchema(schema).addValues(objects).build(), ts(timestamp));
  }
  ...
  private PubsubMessage message(
    ...
    return new PubsubMessage(payload.getBytes(UTF_8), attributes);
  }
